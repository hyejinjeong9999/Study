...
# displ(배기량)이 4 이하인 자동차와 4 초과과인 자동차 중 
# 어떤 자동차의 hwy(고속도로 연비)가 평균적으로 더 높은지 확인하세요.

# 새로운 컬럼을 만듦 displ_diff <= 4 면 LOW, 아니면 High 값 넣기
df <- mutate(mpg, DISPL_DIFF = ifelse(displ <=4,"LOW","HIGH")) %>%
    group_by(DISPL_DIFF)%>%  # displ_diff 로 그룹만들기 high / low 로 나뉨
    summarise(AVG_HWY = mean(hwy)) # AVG_HWY 라는 값 - hwy 평균을 입력
