...
public class Q03_IntStackX {
	...
	// 생성자
	// A는 배열 앞, B는 배열 맨 끝
	public Q03_IntStackX(int capacity) {
		ptrA = 0;
		ptrB = capacity - 1;
		max = capacity;
		try {
	...
	// push
	public int push(AorB sw, int x) throws EmptyIntStackX2Exception {
		// 스택이 가득 찬 경우
		if (ptrA >= ptrB + 1) {
	...
	public int pop(AorB sw) throws EmptyIntStackX2Exception {
		...
		switch (sw) {
		case StackA:
			if (ptrA <= 0) // stack A가 비어있을 경우
				throw new EmptyIntStackX2Exception();
			x = stk[--ptrA]; // 값을 꺼냄
			break;
		case StackB:
			if (ptrB >= max - 1) // stackB가 비어있을 경우
				throw new EmptyIntStackX2Exception();
			x = stk[++ptrB]; // 값을 꺼냄
			break;
		}
